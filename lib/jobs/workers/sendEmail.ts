...
export async function handleSendEmail(job: Job) {
  ...
  try {
    ...
    if (!resend) {
      console.warn('[EMAIL] Resend not configured, skipping send for template:', template)
      return
    }
    ...
    console.log(`[EMAIL] Successfully sent ${template} to ${to}`, { messageId: result.data?.id })
  } catch (err) {
