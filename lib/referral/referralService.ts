...
export async function awardReferralBonus(referredUserId: string): Promise<boolean> {
  ...
  const [{ data: referrerProfile }, { data: referredProfile }] = await Promise.all([
    supabase.from('profiles').select('credit_balance').eq('id', referral.referrer_id).single(),
    supabase.from('profiles').select('credit_balance').eq('id', referral.referred_id).single(),
  ])

  // Add credit to referrer
  const { error: referrerError } = await supabase
    .from('profiles')
    .update({ credit_balance: (referrerProfile?.credit_balance ?? 0) + BONUS_AMOUNT })
    .eq('id', referral.referrer_id)

  // Add credit to referred user
  const { error: referredError } = await supabase
    .from('profiles')
    .update({ credit_balance: (referredProfile?.credit_balance ?? 0) + BONUS_AMOUNT })
