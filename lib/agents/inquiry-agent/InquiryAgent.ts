...
export class InquiryAgent extends BaseAgent {
  ...
  private async createInquiry(
    ...
    try {
      ...
      this.log('create_inquiry_error', { error: (error as any)?.message })
      return {
        success: false,
        error: (error as any)?.message || 'Failed to create inquiry',
        handledBy: this.type,
        durationMs: Date.now() - startTime,
      }
    }
  ...
  private async listInquiries(userId: string): Promise<AgentResponse> {
    ...
    try {
      ...
      this.log('list_inquiries_error', { error: (error as any)?.message })
      return {
        success: false,
        error: (error as any)?.message || 'Failed to list inquiries',
        handledBy: this.type,
        durationMs: Date.now() - startTime,
      }
    }
  ...
  private async closeInquiry(
    ...
    try {
      ...
      this.log('close_inquiry_error', { error: (error as any)?.message })
      return {
        success: false,
        error: (error as any)?.message || 'Failed to close inquiry',
        handledBy: this.type,
        durationMs: Date.now() - startTime,
      }
    }
