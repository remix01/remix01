...
const repeatTracker = new Map<string, { byHash: Map<string, number[]> }>()
...
export async function executeSandboxCode(
  ...
  const repeated = repeatTracker.get(input.userId) ?? { byHash: new Map<string, number[]>() }
  const now = Date.now()
  const codeHash = hashCode(input.code)
  const sameHashRuns = (repeated.byHash.get(codeHash) ?? []).filter((n) => now - n < REPEATED_WINDOW_MS)
  if (sameHashRuns.length >= REPEATED_LIMIT) {
    throw new SandboxPolicyError('QUOTA_EXCEEDED', 'Too many repeated executions. Please wait.', 429)
  }
  sameHashRuns.push(now)
  repeated.byHash.set(codeHash, sameHashRuns)
  for (const [trackedHash, timestamps] of repeated.byHash.entries()) {
    const pruned = timestamps.filter((n) => now - n < REPEATED_WINDOW_MS)
    if (pruned.length === 0) repeated.byHash.delete(trackedHash)
    else repeated.byHash.set(trackedHash, pruned)
  }
  ...
  if (connecting) {
    ...
    sandbox = await (Sandbox as any).connect(input.sandboxId, {
      apiKey: env.E2B_API_KEY,
      timeoutMs: policy.maxRuntimeMs,
    })
  } else {
  ...
  usageTracker.set(input.userId, { day, runs: nextRuns })
  ...
  try {
    ...
    if (sandbox?.sandboxId && activeMap.has(sandbox.sandboxId)) {
      activeMap.set(sandbox.sandboxId, Date.now())
      sessionTracker.set(input.userId, activeMap)
    }
  }
}
