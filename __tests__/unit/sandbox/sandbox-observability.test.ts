...
describe('sandbox observability persistence', () => {
  ...
  it('writes execution and updates aggregates', async () => {
    ...
    const single = jest.fn().mockResolvedValue({ data: { runtime_total_ms: 1, execution_count: 1 } })
    const updateEq = jest.fn().mockResolvedValue({})
    const eqSelect = { maybeSingle, single }
    const update = jest.fn(() => ({ eq: updateEq }))
    fromMock
      .mockReturnValueOnce({ select: () => ({ eq: () => eqSelect }) })
      .mockReturnValueOnce({ insert })
      .mockReturnValueOnce({ select: () => ({ eq: () => ({ single }) }) })
      .mockReturnValueOnce({ update })
    rpcMock.mockRejectedValue(new Error('no rpc'))
