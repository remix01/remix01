...
import { getPartnerDesktopNav, isPartnerNavItemActive } from '@/components/partner/nav-config'
...
export function PartnerSidebar({ partner }: PartnerSidebarProps) {
  ...
  const allNavHrefs = useMemo(() => navItems.map((item) => item.href), [navItems])
      ...
      {navItems.map((item) => (
        <Link
          ...
          className={`flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
            isPartnerNavItemActive(pathname, item.href, allNavHrefs)
