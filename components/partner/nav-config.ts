...
export const isPartnerNavItemActive = (pathname: string | null, href: string, allHrefs: string[]) => {
  if (!pathname) return false
  if (pathname === href) return true
  if (!pathname.startsWith(`${href}/`)) return false

  const hasMoreSpecificActiveMatch = allHrefs.some((candidateHref) => {
    if (candidateHref === href) return false
    if (!candidateHref.startsWith(`${href}/`)) return false
    return pathname === candidateHref || pathname.startsWith(`${candidateHref}/`)
  })

  return !hasMoreSpecificActiveMatch
}
