...
export function SandboxModule() {
  ...
  const highlightedCode = useMemo(() => latestCode, [latestCode])
  ...
  const runCode = async () => {
    ...
    try {
      const res = await fetch('/api/sandbox/execute', {
        ...
        body: JSON.stringify({ code: latestCode, language: template, sandboxId, sandboxToken }),
      })

      const payload = await res.json()
      if (payload.sandboxId) setSandboxId(payload.sandboxId)
      if (payload.sandboxToken) setSandboxToken(payload.sandboxToken)
          ...
          <Tabs defaultValue="code" className="flex h-full flex-col">
            ...
            <TabsContent value="code" className="mt-3 flex-1">
              <ScrollArea className="h-[55vh] rounded-md border bg-slate-950 p-3 text-slate-50">
                <pre><code>{highlightedCode || '/* Tukaj se bo prikazala generirana koda */'}</code></pre>
              </ScrollArea>
