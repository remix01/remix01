      ...
      <Card className="flex min-h-0 flex-col">
        ...
        <CardContent className="min-h-0 flex-1">
          <Tabs defaultValue="code" className="flex h-full flex-col">
            <TabsList>
              <TabsTrigger value="code">Koda</TabsTrigger>
              <TabsTrigger value="output">Izhod</TabsTrigger>
            </TabsList>
              ...
              <div className="mt-3 flex gap-2">
                <Dialog>
                  <DialogTrigger asChild>
                    <Button disabled={!latestCode || running}>{running ? 'Poganjam…' : 'Zaženi'}</Button>
                  </DialogTrigger>
                  <DialogContent>
                    <DialogHeader>
                      <DialogTitle>Potrditev izvajanja kode</DialogTitle>
                      <DialogDescription>
                        Preveri povzetek in potrdi izvajanje kode v E2B sandboxu.
                      </DialogDescription>
                    </DialogHeader>
                    <ScrollArea className="max-h-64 rounded-md border p-2 text-xs">
                      <pre className="whitespace-pre-wrap">{latestCode}</pre>
                    </ScrollArea>
                    <DialogFooter>
                      <Button onClick={runCode}>Potrdi in zaženi</Button>
                    </DialogFooter>
                  </DialogContent>
                </Dialog>
                <Button variant="secondary" onClick={() => {
                  ...
                  Shrani kodo v projekt
                </Button>
              </div>
            </TabsContent>
            <TabsContent value="output" className="mt-3 space-y-2">
              <div className="rounded-md border p-3">
                <p className="mb-1 text-xs font-semibold uppercase">stdout</p>
                <pre className="whitespace-pre-wrap text-sm">{stdout || '—'}</pre>
              </div>
              <div className="rounded-md border p-3">
                <p className="mb-1 text-xs font-semibold uppercase text-destructive">stderr</p>
                <pre className="whitespace-pre-wrap text-sm">{stderr || '—'}</pre>
              </div>
            </TabsContent>
          </Tabs>
        </CardContent>
