...
export async function GET(request: NextRequest) {
  try {
    ...
    let analyticsCheckFromIso = sevenDaysAgo.toISOString()
    if (filters.dateRange !== 'custom') {
      const days = filters.dateRange === '30d' ? 30 : filters.dateRange === '90d' ? 90 : 7
      const from = new Date()
      from.setDate(from.getDate() - (days - 1))
      analyticsCheckFromIso = from.toISOString()
    }
    const analyticsCheckQuery = supabaseAdmin
      .from('analytics_events')
      .select('*', { count: 'exact', head: true })
      .gte('created_at', analyticsCheckFromIso)
