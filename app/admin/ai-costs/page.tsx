...
export default async function AiCostsPage({ searchParams }: { searchParams: Promise<{ range?: string }> }) {
  ...
  const [{ data: totals }, { data: todayData }, { data: byDay }, { data: recentJobs }, { count: failedCount }] =
    await Promise.all([
      supabaseAdmin.from('agent_cost_summary').select('agent_type,messages,tokens_in,tokens_out,cost_usd').gte('date', sinceISO),
      supabaseAdmin.from('agent_cost_summary').select('agent_type,messages,cost_usd').eq('date', today),
      supabaseAdmin.from('agent_cost_summary').select('date,agent_type,messages,tokens_in,tokens_out,cost_usd').gte('date', sinceISO).order('date', { ascending: true }),
  ...
  const data = {
    ...
    today_cost: (todayData ?? []).reduce((s, r) => s + Number(r.cost_usd), 0),
    today_messages: (todayData ?? []).reduce((s, r) => s + Number(r.messages), 0),
    by_agent: byAgent,
    by_day: byDay ?? [],
